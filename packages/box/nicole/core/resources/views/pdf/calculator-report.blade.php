...
<!-- Шапка документа с реквизитами компании из конфига -->
<table style="width: 100%; border-collapse: collapse; margin-bottom: 25px;">
  <tr>
    ...
    <td style="vertical-align: top; text-align: right; width: 50%;">
      <div style="font-size: 12px; color: #9ca3af; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px;">Документ</div>
      <div style="font-size: 14px; font-weight: bold; color: #111827;">КП № {{ $order->kp_number }}</div>
      <div style="font-size: 12px; color: #4b5563; margin-top: 4px;">Дата: {{ $order->created_at ? $order->created_at->format('d.m.Y') : date('d.m.Y') }} г.</div>
    </td>
  </tr>
</table>
...
<!-- СВОДНАЯ СТРУКТУРИРОВАННАЯ ТАБЛИЦА СМЕТЫ -->
<table style="width: 100%; border-collapse: collapse; font-size: 12px;">
  ...
  <tbody>
  <!-- ПЕРЕБИРАЕМ СЕКЦИИ (ИЗДЕЛИЯ) ЗАКАЗА -->
  @foreach($order->sections as $index => $section)
    <!-- Аккуратная серая плашка-заголовок изделия -->
    <tr style="background-color: #e5e7eb; font-weight: bold; border-top: 1px solid #d1d5db; border-bottom: 1px solid #d1d5db;">
      <td colspan="2" style="padding: 10px; color: #111827; text-transform: uppercase; letter-spacing: 0.5px;">
        ...
        @php
          // Ищем наименование камня в характеристиках
    ...
    <!-- ПЕРЕБИРАЕМ СТРОКИ СМЕТЫ ДЛЯ ЭТОГО ИЗДЕЛИЯ -->
    @foreach($section->items as $item)
      <tr style="border-bottom: 1px solid #f3f4f6;">
        <td style="padding: 8px 15px; color: #374151;">{{ $item->name }}</td>
